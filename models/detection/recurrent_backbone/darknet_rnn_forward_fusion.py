...
class RNNDetector(BaseDetector):
    def __init__(self, mdl_config: DictConfig):
        ...
        initial_stage = RNNFusionDetectorStage(dim_in_ev=ev_in_channels,
                    ...
                    resnet_layers=mdl_config.resnet_layers)
    ...
    def forward(self, ev_input: th.Tensor, img_input: th.tensor, prev_states: Optional[LstmStates] = None, token_mask: Optional[th.Tensor] = None, 
        ...
        for stage_idx, stage in enumerate(self.stages):
            if stage_idx == 0:
                x, state, loss_align, feature_alighed, feature_unalighed = stage(ev_input, img_input, prev_states[stage_idx], token_mask if stage_idx == 0 else None, drift=drift)
            else:
...
class RNNFusionDetectorStage(nn.Module):
    ...
    def forward_ssm(self, ev_input: th.Tensor,
        ...
        ev_input = self.ev_initial_layers(ev_input)
        img_input = self.img_initial_layers(img_input)
        ...
        x = self.fusion_block(img_input_aligned, ev_input) #[B, C, H, W] + [B, C, H, W] -> [B, C, H, W] #the former is more important
...
class RNNDetectorStage(nn.Module):
    ...
    def __init__(self,
        ...
        if dim_in < 256:
            self.darknet_layer = self.make_group_layer(dim_in, stage_dim)

        elif dim_in == 256:
            self.darknet_layer = self.make_spp_block(dim_in, stage_dim)
