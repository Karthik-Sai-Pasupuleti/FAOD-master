...
LABELMAP_PKU_FUSION = ("car", "pedestrian", 'two wheeler')
LABELMAP_DSEC = ('pedestrian', 'rider', 'car', 'bus', 'truck', 'bicycle', 'motorcycle', 'train')

def make_binary_histo(events, img=None, width=346, height=260):
    """
...
def draw_bboxes_bbv(img, boxes, labelmap=LABELMAP_PKU_FUSION) -> np.ndarray:
    """
    ...
    if labelmap == LABELMAP_PKU_FUSION:
        classid2colors = {
            0: (255, 255, 0),  # car -> yellow (rgb)
            1: (0, 0, 255),  # ped -> blue (rgb)
            2: (0, 255, 255),  # 2-wheeler cyan (rgb)
        }
        scale_multiplier = 4
...
def draw_bboxes(img, boxes, labelmap=LABELMAP_PKU_FUSION) -> None:
    """
    draw bboxes in the image img
    """
    colors = cv2.applyColorMap(np.arange(0, 255).astype(np.uint8), cv2.COLORMAP_HSV)
    ...
    for i in range(boxes.shape[0]):
        ...
        cv2.rectangle(img, (int(pt1[0]), int(pt1[1])), (int(pt2[0]), int(pt2[1])), tuple(int(c) for c in color), 1)
