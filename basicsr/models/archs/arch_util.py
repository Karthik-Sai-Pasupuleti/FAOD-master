...
class DCNv2Pack(nn.Module):
    ...
    Ref:
        Delving Deep into Deformable Alignment in Video Super-Resolution.
    """
    def __init__(self, feature_channels, offset_feature_channels):
        super().__init__()
        deform_kernel_size = 3
        deform_groups = 8
        self.conv_offset = nn.Sequential(
            nn.Conv2d(offset_feature_channels,
    ...
    def forward(self, x, feat, input_offsets=None):
        offsets = self.conv_offset(feat)
        if input_offsets is not None:
            offsets = torch.ones_like(offsets, requires_grad=False) * input_offsets


        offset_absmean = torch.mean(torch.abs(offsets))
        if offset_absmean > 50:
            logger = get_root_logger()
            # logger.warning(
            #     f'Offset abs mean is {offset_absmean}, larger than 50.')
            
        x_warped = self.dconv(x, offsets)
        return x_warped
